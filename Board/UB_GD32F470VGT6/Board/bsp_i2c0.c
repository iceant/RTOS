...
static void i2c_bus_reset(void){
    ...
    /*配置SDA为输入GPIO*/
    gpio_mode_set(I2C_SDA_GPIOX, GPIO_MODE_INPUT, GPIO_PUPD_NONE, I2C_SDA_PIN);
    
    /*测试SDA是否为低*/    
    if(RESET==gpio_input_bit_get(I2C_SDA_GPIOX, I2C_SDA_PIN)){
        while(1){
            /*配置SCL为开漏输出*/
            gpio_output_options_set(I2C_SCL_GPIOX, GPIO_OTYPE_OD, GPIO_OSPEED_50MHZ, I2C_SCL_PIN);
            ...
            if(nRetry++>9){
                /*测试SDA是否为高*/
                if(SET==gpio_input_bit_get(I2C_SDA_GPIOX, I2C_SDA_PIN)){
                    /* 配置SDA为开漏输出GPIO */
                    gpio_output_options_set(I2C_SDA_GPIOX, GPIO_OTYPE_OD, GPIO_OSPEED_50MHZ, I2C_SDA_PIN);
                    /* SDA=0 */
                    GPIO_BC(I2C_SDA_GPIOX) |= I2C_SDA_PIN; 
