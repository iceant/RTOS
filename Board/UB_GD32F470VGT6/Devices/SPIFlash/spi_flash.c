...
static sFLASH_IO_T * sFLASH__IO = 0;

#define sFLASH_CS_LOW()  sFLASH__IO->CS_Low()
#define sFLASH_CS_HIGH()  sFLASH__IO->CS_High()
...
uint8_t sFLASH_SendByte(uint8_t byte)
{
    ...
    return sFLASH__IO->SendByte(byte);
}
...
uint16_t sFLASH_SendHalfWord(uint16_t HalfWord)
{
    ...
    return sFLASH__IO->SendHalfWord(HalfWord);
}
...
void sFLASH_WaitForWriteEnd(void)
{
    ...
    } while ((flashstatus & sFLASH_WIP_FLAG) == 1); /* Write in progress */
