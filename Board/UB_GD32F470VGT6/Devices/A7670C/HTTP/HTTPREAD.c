...
{
    return A7670C_RequestWithCmd(__FUNCTION__, Read_Handler, response, os_tick_from_millisecond(timeout_ms), "AT+HTTPREAD?\r\n");
}
...
static A7670C_RxHandler_Result Write_Handler(sdk_ringbuffer_t* buffer, void*ud){
    ...
    if(sdk_ringbuffer_find_str(buffer,0, "+HTTPREAD: 0\r\n")!=-1){
        ...
        if(sdk_ringbuffer_cut(&text, buffer, 0, sdk_ringbuffer_used(buffer), "+HTTPREAD: ", "\r\n")==0){
            sdk_hex_dump("HTTPREAD", buffer->buffer, sdk_ringbuffer_used(buffer));
            response->code = kA7670C_Response_Code_OK;
            response->data_len = (int)sdk_ringbuffer_strtoul(buffer, text.start, 0, 10);
        ...
        , uint32_t timeout_ms)
{
    ...
    return A7670C_RequestWithArgs(__FUNCTION__ ,Write_Handler, response, os_tick_from_millisecond(timeout_ms), "AT+HTTPREAD=%d,%d\r\n", start_offset, byte_size);
}
