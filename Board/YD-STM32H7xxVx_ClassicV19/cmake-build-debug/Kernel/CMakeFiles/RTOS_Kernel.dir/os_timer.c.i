...
# 2 "D:/RTOS/Kernel/os_timer.c" 2
# 29 "D:/RTOS/Kernel/os_timer.c"
...
static void os_timer__add(os_timer_t * timer){
    os_list_t * head;
    if(timer->expire_tick <= 0xFF){
        head = &os_timer__tvroot;
    }else if(timer->expire_tick <= 0x3FFF){
        head = &os_timer__tv[0];
    }else if(timer->expire_tick <= 0xFFFFF){
        head = &os_timer__tv[1];
    }else if(timer->expire_tick <= 0x3FFFFFF){
        head = &os_timer__tv[2];
    }else if(timer->expire_tick <= 0xFFFFFFFF){
        head = &os_timer__tv[3];
    }
    do{ ((&timer->wait_node)->prev)->next = (&timer->wait_node)->next; ((&timer->wait_node)->next)->prev = (&timer->wait_node)->prev; (&timer->wait_node)->next = (&timer->wait_node)->prev = (&timer->wait_node);}while(0);
    do{ ((head)->prev)->next = (&timer->wait_node); (&timer->wait_node)->prev = (head)->prev; (head)->prev = (&timer->wait_node); (&timer->wait_node)->next = (head); }while(0);
}
# 63 "D:/RTOS/Kernel/os_timer.c"
os_err_t os_timer_init(void)
{
    os_size_t i;
    os_timer__current_tick = 0;
    do{ (&os_timer__tvroot)->next = (&os_timer__tvroot)->prev = (&os_timer__tvroot); }while(0);
    for(i=0; i< (sizeof(os_timer__tv)/sizeof((os_timer__tv)[0])); i++){
        do{ (&os_timer__tv[i])->next = (&os_timer__tv[i])->prev = (&os_timer__tv[i]); }while(0);
    }
...
os_err_t os_timer_add(os_timer_t * timer, os_timer_timeout_t timeout, void * userdata, os_tick_t ticks, int flags)
{
    ...
    cpu_lock_unlock(&os_timer__lock);;
...
# 101 "D:/RTOS/Kernel/os_timer.c" 3 4
_Bool 
# 101 "D:/RTOS/Kernel/os_timer.c"
         os_timer_tick(void){
    cpu_lock_lock(&os_timer__lock);;

    register 
# 104 "D:/RTOS/Kernel/os_timer.c" 3 4
            _Bool 
# 104 "D:/RTOS/Kernel/os_timer.c"
                      nee_schedule = 
# 104 "D:/RTOS/Kernel/os_timer.c" 3 4
                                     0
# 104 "D:/RTOS/Kernel/os_timer.c"
    ...
    if(current_tick <= 0xFF){
        head = &os_timer__tvroot;
    }else if(current_tick <= 0x3FFF){
        head = &os_timer__tv[0];
    }else if(current_tick <= 0xFFFFF){
        head = &os_timer__tv[1];
    }else if(current_tick <= 0x3FFFFFF){
        head = &os_timer__tv[2];
    }else if(current_tick <= 0xFFFFFFFF){
        head = &os_timer__tv[3];
    }
    ...
    for(node = (head)->next; node!=head; ){
        timer = ((os_timer_t *)((char *)(node) - (os_uint_t)(&((os_timer_t *)0)->wait_node)));
        node = (node)->next;
        if(timer->expire_tick <= current_tick){
            nee_schedule = 
# 135 "D:/RTOS/Kernel/os_timer.c" 3 4
                          1
# 135 "D:/RTOS/Kernel/os_timer.c"
                                 ;
            if(timer->timeout){
...
os_err_t os_timer_node_init(os_timer_t * timer)
{
    do{ (&timer->wait_node)->next = (&timer->wait_node)->prev = (&timer->wait_node); }while(0);
    timer->expire_tick = 0;
    timer->ticks = 0;
    timer->flags = 0;
    timer->userdata = 0;
