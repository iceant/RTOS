...
# 12 "D:/RTOS/Kernel/os_thread.h" 2
...
# 8 "D:/RTOS/Kernel/os_thread.c" 2




# 11 "D:/RTOS/Kernel/os_thread.c"
static void os_thread__exit(os_thread_t * thread){
    ...
    do{ ((&thread->timer_node.wait_node)->prev)->next = (&thread->timer_node.wait_node)->next; ((&thread->timer_node.wait_node)->next)->prev = (&thread->timer_node.wait_node)->prev; (&thread->timer_node.wait_node)->next = (&thread->timer_node.wait_node)->prev = (&thread->timer_node.wait_node);}while(0);
}
        ...
        , os_tick_t init_ticks)
{
    if(name){
        ...
        name_size = (((name_size) > (8 -1))?(8 -1):(name_size));
        printf("name_size:%d\r\n", name_size);
        memcpy(thread->name, name, name_size);
        thread->name[name_size]='\0';
    }

    do{ (&thread->ready_node)->next = (&thread->ready_node)->prev = (&thread->ready_node); }while(0);
    do{ (&thread->wait_node)->next = (&thread->wait_node)->prev = (&thread->wait_node); }while(0);
...
os_err_t os_thread_startup(os_thread_t * thread){
    ...
    return os_scheduler_schedule((2));
}
