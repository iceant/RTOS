# 0 "D:/RTOS/Kernel/os_scheduler.c"
# 1 "D:\\RTOS\\Board\\YD-STM32H7xxVx_ClassicV19\\cmake-build-debug//"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "D:/RTOS/Kernel/os_scheduler.c"
...
# 8 "D:/RTOS/Kernel/os_scheduler.h" 2
...
# 12 "D:/RTOS/Kernel/os_scheduler.h" 2



# 1 "D:/RTOS/Kernel/os_thread.h" 1
# 15 "D:/RTOS/Kernel/os_thread.h"
...
# 16 "D:/RTOS/Kernel/os_thread.h" 2
...
# 20 "D:/RTOS/Kernel/os_thread.h" 2
...
# 24 "D:/RTOS/Kernel/os_thread.h" 2



# 1 "D:/RTOS/Kernel/os_timer.h" 1
# 20 "D:/RTOS/Kernel/os_timer.h"
# 1 "D:/RTOS/Kernel/os_macros.h" 1
# 21 "D:/RTOS/Kernel/os_timer.h" 2
...
# 28 "D:/RTOS/Kernel/os_thread.h" 2
...
# 16 "D:/RTOS/Kernel/os_scheduler.h" 2
...
# 2 "D:/RTOS/Kernel/os_scheduler.c" 2
...
# 3 "D:/RTOS/Kernel/os_scheduler.c" 2
...
# 7 "D:/RTOS/Kernel/os_scheduler.c" 2
...
# 8 "D:/RTOS/Kernel/os_scheduler.c" 2
...
# 9 "D:/RTOS/Kernel/os_scheduler.c" 2




# 12 "D:/RTOS/Kernel/os_scheduler.c"
static os_list_t os_scheduler__ready_list[32]={0};
...
__attribute__((always_inline)) static inline os_thread_t * os_scheduler__pop_highest(void){
    ...
    os_list_node_t *node = (head)->next;
    os_thread_t * thread = ((os_thread_t *)((char *)(node) - (os_uint_t)(&((os_thread_t *)0)->ready_node)));
    do{ ((&thread->ready_node)->prev)->next = (&thread->ready_node)->next; ((&thread->ready_node)->next)->prev = (&thread->ready_node)->prev; (&thread->ready_node)->next = (&thread->ready_node)->prev = (&thread->ready_node);}while(0);
    if(((head)->next==(head))){
        os_priority_unmark(highest_priority);
    }
    return thread;
...
__attribute__((always_inline)) static inline void os_scheduler__pending_list_push_back(os_thread_t * thread){
    if(thread==0)return;
    ...
    do{ ((&thread->ready_node)->prev)->next = (&thread->ready_node)->next; ((&thread->ready_node)->next)->prev = (&thread->ready_node)->prev; (&thread->ready_node)->next = (&thread->ready_node)->prev = (&thread->ready_node);}while(0);
    do{ ((&os_scheduler__pending_list)->prev)->next = (&thread->ready_node); (&thread->ready_node)->prev = (&os_scheduler__pending_list)->prev; (&os_scheduler__pending_list)->prev = (&thread->ready_node); (&thread->ready_node)->next = (&os_scheduler__pending_list); }while(0);
}

__attribute__((always_inline)) static inline void os_scheduler__ready_list_push(os_thread_t * thread, os_scheduler_push_type_enum pushType){
    if(thread==0)return;
    os_priority_t priority = thread->curr_priority;
    os_list_t * head = &os_scheduler__ready_list[priority];
    os_list_node_t * node;
    for(node = (head)->next; node!=head;){
        os_thread_t* p = ((os_thread_t *)((char *)(node) - (os_uint_t)(&((os_thread_t *)0)->ready_node)));
        if(p==thread){
            return;
        }
        node = (node)->next;
    ...
    if(pushType==kOsSchedulerPushType_BACK){
        do{ ((head)->prev)->next = (&thread->ready_node); (&thread->ready_node)->prev = (head)->prev; (head)->prev = (&thread->ready_node); (&thread->ready_node)->next = (head); }while(0);
    }else{
        do{ ((head)->next)->prev = (&thread->ready_node); (&thread->ready_node)->next = (head)->next; (head)->next = (&thread->ready_node); (&thread->ready_node)->prev = (head); }while(0);
    }
...
static void os_scheduler__timer_timeout(os_timer_t * timer, void* userdata){
    ...
    os_thread_t* thread = ((os_thread_t *)((char *)(timer) - (os_uint_t)(&((os_thread_t *)0)->timer_node)));
...
os_err_t os_scheduler_init(void){
    os_size_t i;
    for(i=0; i< (sizeof(os_scheduler__ready_list)/sizeof((os_scheduler__ready_list)[0])); i++){
        do{ (&os_scheduler__ready_list[i])->next = (&os_scheduler__ready_list[i])->prev = (&os_scheduler__ready_list[i]); }while(0);
    }
    do{ (&os_scheduler__yield_list)->next = (&os_scheduler__yield_list)->prev = (&os_scheduler__yield_list); }while(0);
    do{ (&os_scheduler__pending_list)->next = (&os_scheduler__pending_list)->prev = (&os_scheduler__pending_list); }while(0);

    os_scheduler__current_tick = 0;
    os_scheduler__current_thread = 0;
    os_scheduler__schedule_nest = 0;
    cpu_lock_init(&os_scheduler__lock);
    os_scheduler__state = (1);
...
void os_scheduler_timed_wait(os_thread_t * thread, os_tick_t ticks){
    if(thread==0) return;
    cpu_lock_lock(&os_scheduler__lock);
    ...
    thread->state = 6;
    os_timer_add(&thread->timer_node, os_scheduler__timer_timeout, thread, ticks, (1<<0));
    cpu_lock_unlock(&os_scheduler__lock);
}

__attribute__((always_inline)) static inline void os_scheduler_mark_skipped(void)
{
...
void os_scheduler_on_systick(void){
    ...
    register 
# 145 "D:/RTOS/Kernel/os_scheduler.c" 3 4
            _Bool 
# 145 "D:/RTOS/Kernel/os_scheduler.c"
                      curr_thread_need_schedule = 
# 145 "D:/RTOS/Kernel/os_scheduler.c" 3 4
                                                  0
# 145 "D:/RTOS/Kernel/os_scheduler.c"
                                                          ;
    register 
# 146 "D:/RTOS/Kernel/os_scheduler.c" 3 4
            _Bool 
# 146 "D:/RTOS/Kernel/os_scheduler.c"
                      timer_need_schedule = 
# 146 "D:/RTOS/Kernel/os_scheduler.c" 3 4
                                            0
# 146 "D:/RTOS/Kernel/os_scheduler.c"
    ...
    if(curr_thread!=0){
        if(curr_thread->state==3){
            curr_thread->remain_ticks--;
            if(curr_thread->remain_ticks == 0){
                curr_thread->state = 4;
                curr_thread_need_schedule = 
# 157 "D:/RTOS/Kernel/os_scheduler.c" 3 4
                                           1
# 157 "D:/RTOS/Kernel/os_scheduler.c"
                                                  ;
            }
        }
    }
    ...
    if(os_scheduler_skipped()>0U && os_interrupt_nest()==0U){
        ...
        if(curr_thread!=0 && curr_thread->state==3){
            curr_thread->state = 4;
        }
        cpu_lock_unlock(&os_scheduler__lock);
        os_scheduler_schedule((1));
        return;
    }

    if(curr_thread_need_schedule==
# 174 "D:/RTOS/Kernel/os_scheduler.c" 3 4
                                 1
# 174 "D:/RTOS/Kernel/os_scheduler.c"
                                        ){
        if(os_interrupt_nest()>0U){
            ...
            cpu_lock_unlock(&os_scheduler__lock);
            os_scheduler_schedule((2));
            return;
        }
    }

    if(timer_need_schedule==
# 185 "D:/RTOS/Kernel/os_scheduler.c" 3 4
                           1
# 185 "D:/RTOS/Kernel/os_scheduler.c"
                                  ){
        if(os_interrupt_nest()==0U){
            cpu_lock_unlock(&os_scheduler__lock);
            os_scheduler_schedule((2));
...
os_err_t os_scheduler_schedule(int policy)
{
    ...
    curr_thread = os_scheduler__current_thread;
    if(curr_thread!=0){
        if(curr_thread->state==3 && curr_thread->remain_ticks!=0){
...
os_err_t os_scheduler_suspend(os_thread_t * thread){
    ...
    thread->remain_ticks = 0;
    do{ ((&thread->ready_node)->prev)->next = (&thread->ready_node)->next; ((&thread->ready_node)->next)->prev = (&thread->ready_node)->prev; (&thread->ready_node)->next = (&thread->ready_node)->prev = (&thread->ready_node);}while(0);
    do{ ((&thread->wait_node)->prev)->next = (&thread->wait_node)->next; ((&thread->wait_node)->next)->prev = (&thread->wait_node)->prev; (&thread->wait_node)->next = (&thread->wait_node)->prev = (&thread->wait_node);}while(0);
...
os_err_t os_scheduler_yield(os_thread_t * thread)
{
    ...
    thread->state = 4;
    do{ ((&thread->ready_node)->prev)->next = (&thread->ready_node)->next; ((&thread->ready_node)->next)->prev = (&thread->ready_node)->prev; (&thread->ready_node)->next = (&thread->ready_node)->prev = (&thread->ready_node);}while(0);
    do{ ((&os_scheduler__yield_list)->prev)->next = (&thread->ready_node); (&thread->ready_node)->prev = (&os_scheduler__yield_list)->prev; (&os_scheduler__yield_list)->prev = (&thread->ready_node); (&thread->ready_node)->next = (&os_scheduler__yield_list); }while(0);
    cpu_lock_unlock(&os_scheduler__lock);
    return (0);
}
