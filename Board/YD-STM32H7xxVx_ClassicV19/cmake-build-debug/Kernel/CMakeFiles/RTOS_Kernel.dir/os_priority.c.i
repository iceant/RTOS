# 0 "D:/RTOS/Kernel/os_priority.c"
# 1 "D:\\RTOS\\Board\\YD-STM32H7xxVx_ClassicV19\\cmake-build-debug//"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "D:/RTOS/Kernel/os_priority.c"
...
# 8 "D:/RTOS/Kernel/os_priority.h" 2
...
# 2 "D:/RTOS/Kernel/os_priority.c" 2
...
# 4 "D:/RTOS/Kernel/os_priority.c" 2
# 12 "D:/RTOS/Kernel/os_priority.c"

# 12 "D:/RTOS/Kernel/os_priority.c"
static os_uint_t os_priority__table[(((32 -1)/(8*sizeof(os_uint_t)))+1)];
...
{
    os_size_t i;
    for(i=0; i<(((32 -1)/(8*sizeof(os_uint_t)))+1); i++){
...
os_priority_t os_priority_get_highest(void){
    ...
    for(int i=0; i<(((32 -1)/(8*sizeof(os_uint_t)))+1); i++){
        table_p = &os_priority__table[i];
        if(table_p==0){
            priority+=(8*sizeof(os_uint_t));
        }else{
            break;
        }
    }

    priority += (os_priority_t)(uint8_t)__builtin_clz(cpu_RBIT(*table_p));
    return (priority==32)?(-1):(priority);
}

void os_priority_mark(os_priority_t priority)
{
# 62 "D:/RTOS/Kernel/os_priority.c"
    ((uint8_t *)os_priority__table)[priority/8]|=(1<<(priority % 8));
}

void os_priority_unmark(os_priority_t priority)
{
# 78 "D:/RTOS/Kernel/os_priority.c"
    ((uint8_t *)os_priority__table)[priority/8] &= ~(1<<(priority % 8));
}


# 81 "D:/RTOS/Kernel/os_priority.c" 3 4
_Bool 
# 81 "D:/RTOS/Kernel/os_priority.c"
    os_priority_is_marked(os_priority_t priority){
