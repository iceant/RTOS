...
C_STATIC_FORCEINLINE void cpu_interrupt_disable(cpu_interrupt_context_t* context){
    cpu_uint_t level = cpu_get_primask();
    cpu_disable_irq();
    *context = level;
}

C_STATIC_FORCEINLINE void cpu_interrupt_enable(const cpu_interrupt_context_t* level){
